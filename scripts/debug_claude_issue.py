...
async def main():
    ...
    try:
        ...
        if old_client_data:
            print(f"✅ Old client ID still exists in Redis!")
            ...
            for key in keys:
                ...
                if client_data:
                    try:
                        ...
                        if ('claude' in client.get('client_name', '').lower() or
                            'claude.ai' in str(client.get('redirect_uris', []))):
        ...
        for client in sorted(claude_clients, key=lambda x: x.get('created', 0)):
            ...
            if client['created']:
                print(f"Created: {datetime.fromtimestamp(client['created']).isoformat()}")
