...
def hunt_regex_patterns(content: str, file_path: Path) -> List[Dict[str, Any]]:
    ...
    for match in config_class_pattern.finditer(content):
        ...
        violations.append({
            ...
            "message": f"🔥 REGEX CATCH: 'class Config:' pattern detected! Use 'model_config = ConfigDict()' instead! ⚡",
            "severity": "error",
            "class_name": "Unknown"
        })
        ...
        for i, line in enumerate(lines):
            if 'pydantic' in line and ('import' in line or 'from' in line):
                violations.append({
                    ...
                    "line": i + 1,
                    "column": 0,
                    "message": f"⚠️  REGEX PREVENTION: File uses Pydantic but doesn't import ConfigDict! Consider adding 'from pydantic import ConfigDict'! ⚡",
                    "severity": "warning",
                    "class_name": "N/A"
                })
                break
