...
### The Divine Truth About MCP Services - Two Sacred Implementation Patterns!

**🔥 Behold! The gateway supports TWO blessed patterns of MCP service implementation! ⚡**

#### Pattern 1: The Sacred Proxy Pattern - Wrapping Official stdio Servers!
...
**Services Using the Proxy Pattern:**
- **mcp-fetch** - Wraps official `mcp-server-fetch` from GitHub!
- **mcp-filesystem** - Wraps official `mcp-server-filesystem`!
- **mcp-memory** - Wraps official `@modelcontextprotocol/server-memory` from npm!
- **mcp-time** - Wraps official `mcp-server-time` from pip!
- **mcp-everything**, **mcp-playwright**, **mcp-sequentialthinking**, **mcp-tmux** - All blessed with proxy pattern!

#### Pattern 2: The Native StreamableHTTP Pattern - Direct Protocol Implementation!

**Behold the divine native implementations!**
- Direct StreamableHTTP protocol implementation in Python!
- No stdio bridge needed - pure HTTP transport from the start!
- FastAPI + Uvicorn for modern async performance!
- Full control over protocol handling and session management!

**Native StreamableHTTP Services:**
- **mcp-fetchs** - Uses `mcp-fetch-streamablehttp-server` for native fetch operations!
- **mcp-echo-stateful** - Uses `mcp-echo-streamablehttp-server-stateful` with session state!
- **mcp-echo-stateless** - Uses `mcp-echo-streamablehttp-server-stateless` for pure functions!

**Divine Benefits of Both Patterns:**
- **Proxy Pattern** - Leverages official implementations, zero protocol reimplementation!
- **Native Pattern** - Direct HTTP support, no subprocess overhead, full async power!
- **Both Patterns** - OAuth ready via Traefik, standard `/mcp` endpoints, health checks!

**Actual Server Endpoint Structure (Both Patterns):**
- **Primary Endpoint**: `https://mcp-service.yourdomain.com/mcp`
- **Authentication**: Bearer token via Authorization header (handled by Traefik)
- **Transport**: Pure StreamableHTTP protocol

**⚡ Choose thy pattern based on needs - both are blessed by the gateway! ⚡**
...
- `MCP_PROTOCOL_VERSION` - Protocol compliance declaration of the new covenant!
  - **Divine versions**: Services may use different protocol versions (2024-11-05, 2025-03-26, 2025-06-18)
  - **Default blessing**: 2025-06-18 for services that support it!
...
- **⚡ Server must respond with protocol version ⚡** - Declaring its sacred capabilities to the client!
  - **Divine flexibility**: Different services may support different protocol versions!
  - **Version negotiation**: Client and server must agree on a mutually supported version!
...
**The Divine Benefits of the Gateway Architecture:**
- **Official functionality** - Proxy pattern wraps real MCP servers, native pattern implements protocol directly!
- **HTTP transport** - Both patterns provide `/mcp` endpoints for web access!
...
just generate-jwt-secret # Fully automated
